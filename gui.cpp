...
namespace Gui
{
	...
	void save_config(char* cfgname) {
		...
		str = (std::string(_("keypsilent=")) + std::to_string(vars->keybinds.psilent) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keyaimbot=")) + std::to_string(vars->keybinds.aimbot) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keyflywall=")) + std::to_string(vars->keybinds.flywall) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keysilentmelee=")) + std::to_string(vars->keybinds.silentmelee) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keyautoshoot=")) + std::to_string(vars->keybinds.autoshoot) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keymanipulator=")) + std::to_string(vars->keybinds.manipulator) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keydesync_ok=")) + std::to_string(vars->keybinds.desync_ok) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keytimescale=")) + std::to_string(vars->keybinds.timescale) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keysilentwalk=")) + std::to_string(vars->keybinds.silentwalk) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keysuicide=")) + std::to_string(vars->keybinds.suicide) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keyneck=")) + std::to_string(vars->keybinds.neck) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keyzoom=")) + std::to_string(vars->keybinds.zoom) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keybullettp=")) + std::to_string(vars->keybinds.bullettp) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keyfakelag=")) + std::to_string(vars->keybinds.fakelag) + _("\n"));
		f.write(str.c_str(), str.size());
		str = (std::string(_("keytp=")) + std::to_string(vars->keybinds.tp) + _("\n"));
		f.write(str.c_str(), str.size());
	...
	void visual() {
		if (im::BeginChild(_("Indicators"), ImVec2(235, 170), true, ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoMove))
		{
			...
			im::Checkbox(_("Rocket prediction"), &vars->visual.rocketprediction);
